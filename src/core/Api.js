...
class Api {
  ...
  getAllCommands = () =>
    db.get("commands").filter({ isTrash: false }).value().reverse();
  ...
  getAllTags = () =>
    db.get("commands").filter({ isTrash: false }).map("tags").value();

  getCommandsContainsTag = (tag) =>
    db
      .get("commands")
  ...
  queryCommand = (query) => {
    ...
    if (commands && commands.length > 0) {
      query = query && query.length ? query.toLowerCase() : "";
      ...
      const results = miniSearch.search(query, {
        // prefix: true,
        boost: { title: 10, command: 10, tags: 5, description: 1 },
        fuzzy: 0,
